...
async function cargarVuelos(token) {
  try {
    const response = await fetch(
      "http://localhost:8080/api/vuelos/mis-vuelos",
      {
        headers: {
              ...
              <div class="flex items-center gap-4">
                <div class="w-16 h-16 bg-center bg-no-repeat aspect-square bg-cover rounded-lg flex-1"
                  style='background-image: url("data:image/png;base64,${
                    aerolinea.imagen?.base64 || ""
                  }")'>
                </div>
                <div class="flex flex-col">
                  <p class="text-lg font-bold text-text-primary dark:text-white">${ciudadSalida} (${
        vuelo.ciudad_salida.id_ciudad
      }) a ${ciudadLlegada} (${vuelo.ciudad_llegada.id_ciudad})</p>
                  <p class="text-sm text-text-secondary dark:text-gray-400">${
                    vuelo.hora_salida
                  } - ${vuelo.hora_llegada}</p>
                  <p class="text-sm text-text-secondary dark:text-gray-400">${
                    aerolinea.nombre
                  }</p>
                </div>
